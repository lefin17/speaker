...
class EndpointFabric:
    ...
    def __init__(self, options):
        ...
        self.frontLength = frontTimeMS / 1000 * self.freqRate    
    
    def frontMaker(self, frame):
        sellf.front = np.append(self.front, frame, axis=0) # добавляем фрейм к фронту
        if np.size(self, 0) > self.frontLength:
            np.delete(self.front, 0, axis=0) # удаляем первую строку из матрицы фронта 
